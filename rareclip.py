...
class RareCLIP():
    def __init__(self, args) -> None:
        ...
        if self.k_shot > 0:
            self.normal_feature_num_min = min(int(self.H * self.W * self.k_shot / 4), self.sample_num)
            self.normal_weight = self.k_shot / 8
    ...
    ## Sequential Coreset Sampling
    def SCS(self, F_ref=None, S_ref=None, normal_fnum=0):
    ...
    def GCS(self, F_ref=None, S_ref=None, normal_fnum=0):
        if F_ref.shape[0] > self.sample_num:
            keep_idxs = torch.tensor(self.GCSer._compute_greedy_coreset_indices(F_ref, self.sample_num)).to(self.device).long()
            F_ref = F_ref[keep_idxs].contiguous()
            S_ref = S_ref[:, keep_idxs].contiguous()
        return F_ref, S_ref, 0
    
    ## Random Sampling
    def RS(self, F_ref=None, S_ref=None, normal_fnum=0):
        if F_ref.shape[0] > self.sample_num:
            keep_idxs = torch.randperm(F_ref.shape[0], device=self.device)[:self.sample_num]
            F_ref = F_ref[keep_idxs].contiguous()
            S_ref = S_ref[:, keep_idxs].contiguous()
        return F_ref, S_ref, 0
    
    ## K-means Cluster Sampling
    def KCS(self, F_ref=None, S_ref=None, normal_fnum=0, max_iters=20, tol=1e-4):
        if F_ref.shape[0] > self.sample_num:
            n_samples, n_features = F_ref.shape
            k = self.sample_num
            indices = torch.randperm(n_samples, device=self.device)[:k]
            centers = F_ref[indices]
            
            for _ in range(max_iters):
                distances = torch.cdist(F_ref, centers)  # (n_samples, k)
                ...
                new_centers = torch.mm(one_hot.T, F_ref)  # (k, n_features)
            ...
            point_distances = torch.norm(F_ref - assigned_centers, p=2, dim=1)
            sampled_indices = []
            for i in range(k):
                ...
                if mask.any():
                    ...
                    global_distances = torch.norm(F_ref - centers[i], p=2, dim=1)
                    sampled_indices.append(torch.argmin(global_distances))
            
            keep_idxs = torch.stack(sampled_indices)
            F_ref = F_ref[keep_idxs].contiguous()
            S_ref = S_ref[:, keep_idxs].contiguous()
        return F_ref, S_ref, 0
    
    def segment_and_update(self, F, F_ref=None, S_ref=None, normal_fnum=0, update=True):
        anomaly_map, k_shot_map = 0, 0
        if self.tested_num > 0:
            patch2ref = torch.mm(F[0], F_ref.t())
            top_sim, top_idx = torch.topk(patch2ref, k=self.topk, dim=-1, sorted=True)
            top_sim = self.LS(top_sim, self.LS_ratio)
            
            ref_prob = (top_sim * S_ref[:, top_idx]).mean(dim=-1)
            estimate_prob = torch.topk(ref_prob, k=int(np.ceil(ref_prob.shape[0] * self.rare_thd)), dim=0, sorted=False)[0].mean(0)
            anomaly_map = (1 - estimate_prob).unsqueeze(0)

            ref_sim, _ = patch2ref.max(dim=0)
            ref_sim = self.LS(ref_sim, self.LS_ratio)
            
            if self.k_shot:
                k_shot_sim = patch2ref[:, :normal_fnum].max(dim=-1)[0]
                k_shot_sim = self.LS(k_shot_sim, self.LS_ratio)
                k_shot_map = (1 - k_shot_sim).unsqueeze(0)

            if update:
                F_ref = torch.cat((F_ref, F[0]), dim=0)
                ref_prob = torch.cat((ref_prob, torch.ones_like(estimate_prob).unsqueeze(0)), dim=0)
                S_ref = torch.cat((S_ref, ref_sim.unsqueeze(0)), dim=0)
                S_ref = torch.cat((S_ref, ref_prob), dim=-1)
                if self.tested_num < self.k_shot:
                    normal_fnum += F[0].shape[0]
        else:
            F_ref = F[0]
            S_ref = torch.ones(1, F[0].shape[0]).to(F.device)
            if self.tested_num < self.k_shot:
                normal_fnum += F[0].shape[0]
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            for l in self.l_list:
                ...
                if self.tested_num > 0:
                    ...
                    foreground_score += 1 - torch.topk(aaif_sim, k=int(np.ceil(aaif_sim.shape[-1] * self.rare_thd)), sorted=False)[0].mean(-1)
                    if self.Rs and self.tested_num >= self.k_shot:
            ...
            ## Early Termination for k-normal-shot
            if self.tested_num < self.k_shot:
                self.tested_num += 1
                for ri, r in enumerate(self.r_list):
                    for l in self.l_list:
