...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # x_attn = q_x + self.ls_1(tmp)
        # x = x_attn + self.ls_2(self.mlp(self.ln_2(x_attn)))
        # return x, attn, tmp
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9],
        ...
        #     else:
        #         if idx == 12:
        #             x, attn, x_attn = r(x, attn_mask=attn_mask)
        #             out_attn.append(attn)
        #         else:
        #             x, attn_tmp, x_attn = r(x, attn_mask=attn_mask)
        #         if idx in out_layers:
        #             out_tokens.append(x_attn)
        # return x, out_attn, out_tokens
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list):
        ...
        patch_tokens = [patch_tokens[t].permute(1, 0, 2) for t in range(len(patch_tokens))]  # LND -> NLD [:, 1:, :]
